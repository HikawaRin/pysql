...
- [微波元器件饱和特性测试数据存储研究](#微波元器件饱和特性测试数据存储研究)
  - [第一章 背景与意义](#第一章-背景与意义)
  - [第二章 饱和特性测试数据存储](#第二章-饱和特性测试数据存储)
    - [2.1 主要技术](#21-主要技术)
      - [2.1.1 XML及XPath](#211-xml及xpath)
      - [2.1.2 关系数据库管理系统](#212-关系数据库管理系统)
      - [2.1.3 Python](#213-python)
    - [2.2 实现方式](#22-实现方式)
      - [2.2.1 将XML文件转化为关系表](#221-将xml文件转化为关系表)
      - [2.2.2 将转化后的关系表存储到数据库中](#222-将转化后的关系表存储到数据库中)
      - [2.2.3 将数据库中的数据转化为XML文件](#223-将数据库中的数据转化为xml文件)
  - [第三章 总结与展望](#第三章-总结与展望)
  - [参考文献](#参考文献)
...
饱和特性数据包含的数据项非常多，结构十分负责  
如图：  
![饱和特性数据汇总.png](饱和特性数据汇总.png)  
...
本文通过对微波器件饱和特性测试数据结构化存储的研究，提出了一套由将半结构化数据转换为关系型数据并存储至数据库中的算法，证明了大量数据结构化存储的可能性，通过统计实际运用中算法的平均运行时间，得出了对于一次完整的饱和特性测量数据完成一次存储的时间约为0.3s，完成一次复原的时间约为0.5s，初步达到使用要求。该部分内容还有改进与拓展的余地，由于Python语言的特性使得其在运行速度上有所欠缺，可考虑将此算法使用编译型语言（如C++）重写以提升其运行速度；根据课题依托的背景，可在实际运用方面进行拓展，如自动生成测试报表，形成一套针对生产环境中存在的某一具体问题的完整解决方案，切实提高生产力。与此相关的工作已在进行中。  
